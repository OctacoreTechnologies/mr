...
class ApplicationController extends Controller
{
    ...
    private function getFormData(): array
    {
        return [
            ...
            'materialToProcess'       => MaterialToProcess::select('material_to_process')->distinct()->pluck('material_to_process'),
            'rotatingBlades'          => Blade::where('type', 'rotating_blades')->select('no_of_blades')->distinct()->pluck('no_of_blades'),
            'fixedBlades'             => Blade::where('type', 'fix_blades')->select('no_of_blades')->distinct()->pluck('no_of_blades'),
            'blowers'                 => Blower::select('blower')->distinct()->pluck('blower'),
            'rotaryAirLockValves'     => RotaryAirLockValve::select('rotary_air_lock_valve')->distinct()->pluck('rotary_air_lock_valve'),
            'feedingHooperCapacities' => FeedingHooperCapacity::select('feeding_hooper_capacity')->distinct()->pluck('feeding_hooper_capacity'),
            'motorRequirements'       => MototRequirement::select('motor_requirement')->distinct()->pluck('motor_requirement'),
            'makeMotors'              => MakeMotor::select('name')->distinct()->pluck('name'),
            'batchs'                  => Batch::select('batches')->distinct()->pluck('batches'),
            'mixingTools'             => MixingTool::select('mixing_tool')->distinct()->pluck('mixing_tool'),
            'electricalControls'      => ElelctricalControl::select('electrical_control')->distinct()->pluck('electrical_control'),
            'acFrequencyDrives'       => AcFequencyDrive::select('ac_fequency_drive')->distinct()->pluck('ac_fequency_drive'),
            'bearings'                => Bearing::select('bearing')->distinct()->pluck('bearing'),
            'pneumatics'              => Pneumatic::select('pneumatic')->distinct()->pluck('pneumatic'),
            'driveSystems'            => DriveSystem::select('drive_system')->distinct()->pluck('drive_system'),
            'gearboxes'               => GearBox::select('gear_box')->distinct()->pluck('gear_box'),
        ];
    }
