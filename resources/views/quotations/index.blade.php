                ...
                <x-adminlte-datatable id="table1" :heads="$heads" striped hoverable with-buttons>
                    @foreach ($quotations as $quotation)
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-link text-danger" href="{{ route('quotation.pdf', $quotation->id) }}" target="_blank">
                            ...
                            <td>
                                <div class="dropdown">
                                    <button class="btn btn-sm btn-outline-secondary dropdown-toggle shadow" type="button" id="actionsDropdown{{ $quotation->id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="actionsDropdown{{ $quotation->id }}">
                                      ...
                                      {{-- <a class="dropdown-item" href="{{ route('quotation.show', $quotation->id) }}">
                                        ...
                                        <form action="{{ route('quotation.destroy', $quotation->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <button class="dropdown-item text-info" data-toggle="modal" data-target="#updateQuotationStatus"
                                        ...
                                        <button class="dropdown-item text-info" data-toggle="modal" data-target="#sendEmailModal"
                                            data-id="{{ $quotation->id }}"
                                            data-customer_name="{{ $quotation->customer->company_name }}"
        ...
        <form method="POST" action="{{ route('admin.quotation.verify') }}">
            ...
            <div class="row">
                <x-adminlte-input type="hidden" name="id" id="modalQuotationId" />
                  <x-adminlte-input name="customer_name" id="customerName" label="Customer Name" fgroup-class="col-12 mb-3" readonly />
                <x-adminlte-input name="reference_no" id="modalReferenceNo" label="Quotation Reference No." fgroup-class="col-12 mb-3" readonly />
    ...
    <x-adminlte-modal id="sendEmailModal" title="Send Email to Customer" theme="info" icon="fas fa-envelope">
        <form method="POST" action="{{ route('quotation.send.mail') }}">
            ...
            <div class="row">
                ...
                <x-adminlte-input name="customer_name" id="emailCustomerName" label="Customer Name" fgroup-class="col-12 mb-3" readonly />
                <x-adminlte-input type="email" name="customer_email" id="emailCustomerEmail" label="Customer Email" type="email" fgroup-class="col-12 mb-3" readonly />
                <x-adminlte-input type="text" id="subject" name="subject" id="subject" label="Subject" fgroup-class="col-12 mb-3" />
                <x-adminlte-textarea name="message" id="message" label="Message" rows="5" fgroup-class="col-12 mb-3" placeholder="Type your message here..." required />
                <div class="col-12 mb-3">
                     <input  type="checkbox" name="withPdf" id="withPdf" value="1"  label="With Quotation Pdf Send"/><label for="withPdf">Send with Quotation Pdf</label>
                </div>
     ...
     <!-- Quotation Status Update -->
        ...
        <form method="POST" action="{{ route('quotation.update.status') }}">
            ...
            <div class="row">
                ...
                <x-adminlte-input name="customer_name" id="statusCustomerName" label="Customer Name" fgroup-class="col-12 mb-3" readonly />
                <x-adminlte-input name="reference_no" id="referenceNo" label="Reference NO." type="text" fgroup-class="col-12 mb-3" readonly />
