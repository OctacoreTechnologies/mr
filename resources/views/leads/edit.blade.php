                    ...
                    <select name="country" id="country" class="crm-select country-select">
                        @foreach ($countries as $country)
                            <option value="{{ strtolower($country->country) }}"
                                ...
                                {{ old('country', strtolower($lead->country ?? '')) == strtolower($country->country) ? 'selected' : '' }}>
                                {{ $country->country }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="region" id="region" class="crm-select">
                        @foreach ($regions as $region)
                            <option value="{{ $region->name }}"
                                ...
                                {{ old('region', $lead->region) == $region->name ? 'selected' : '' }}>
                                {{ $region->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="state" id="state" class="crm-select">
                        @foreach ($states as $state)
                            <option value="{{ $state->name }}"
                                {{ old('state', strtolower($lead->state ?? '')) == strtolower($state->name) ? 'selected' : '' }}>
                                {{ $state->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="crm-section-body">
            <div class="crm-form-grid crm-form-grid-3">
                <div class="crm-field-wrap">
                    ...
                    <input type="text" name="company_name" id="company_name" value="{{ old('company_name', $lead->company_name) }}" class="crm-input" placeholder="Enter company name">
                </div>
                <div class="crm-field-wrap">
                    <label class="crm-field-label">Company website URL</label>
                    <input type="text" name="company_website" value="{{ old('company_website', $lead->company_website) }}"
            ...
            <div class="crm-form-grid crm-form-grid-3">
                <div class="crm-field-wrap">
                    ...
                    <select name="status" class="crm-select">
                        ...
                        <option value="disqualified" {{ old('status', $lead->status) == 'disqualified' ? 'selected' : '' }}>Disqualified
                        </option>
                    </select>
                </div>
                <div class="crm-field-wrap">
                    ...
                    <select name="followed_by" class="crm-select" required>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('followed_by', $lead->followed_by) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
...
<script>
    @php
        $personsData = old('contact_persons') ? array_values(old('contact_persons')) : ($lead->contact_persons ?? []);
        if (!empty($personsData) && empty($personsData[0]['visiting_cards']) && empty($personsData[0]['visiting_card']) && !empty($lead->visiting_card)) {
            $personsData[0]['visiting_card'] = $lead->visiting_card;
        }
    @endphp
