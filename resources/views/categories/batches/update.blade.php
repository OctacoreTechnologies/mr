                ...
                <div class="row">
                    <div class="col-md-6">
                        <x-adminlte-select name="machine_id" label="Select Machine" id="machine_id">
                            ...
                            @foreach ($machines as $machine)
                                <option value="{{ $machine->id }}"
                                    {{ $machine->id === ($batch->model->machine->id ?? null) ? 'selected' : '' }}>
                                    {{ $machine->name }}</option>
                            @endforeach
                        </x-adminlte-select>
                    </div>
                      <div class="col-md-6">
                        ...
                        <select id="model_id" name="model_id" class="form-control select2 form-control-lg"
                            ...
                            <option value="{{ $batch->model->id??'' }}" selected>{{ $batch->model->name ?? '' }}</option>

                        </select>
                    </div>
                    <div class="col-md-6">
                        <x-adminlte-input type="text" name="batches" value="{{ $batch->batches }}" label="Batch"
                    ...
                    <div class="col-md-12">
                        ...
                        <button type="button" class="btn btn-danger my-2"
                            onclick="window.location.href='{{ route('batch.index') }}'">Cancel</button>
                    </div>
                </div>
