...
@section('content_header')
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        <h1 class="mb-2 mb-md-0">Bearing</h1>
        <div class="d-flex flex-wrap gap-2">
            {{-- Primary Add Button --}}
            <x-adminlte-button label="Add Bearing" theme="success" icon="fas fa-plus" data-toggle="modal" data-target="#modalMin"/>
...
<div class="card shadow-sm">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title">Bearing</h3>
        ...
        <x-adminlte-datatable id="table1" :heads="$heads" striped hoverable bordered compressed>
            @foreach ($bearings as $bearing)
                <tr>
                    ...
                    <td>
                        <nobr>
                            <a href="{{ route('bearing.edit', $bearing->id) }}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit">
                                <i class="fa fa-lg fa-fw fa-pen"></i>
                            </a>
                            <form action="{{ route('bearing.destroy', $bearing->id) }}" method="POST" class="d-inline-block">
                                @csrf
                                @method('DELETE')
...
{{-- Back Link --}}
<a href="{{ route('bearing.index') }}" class="btn btn-outline-primary mt-3">
    <i class="fas fa-arrow-left mr-1"></i> Back to Machines
</a>

{{-- Modal for Add --}}
<x-adminlte-modal id="modalMin" title="Add Bearing" theme="teal" icon="fas fa-plus">
    <form method="POST" action="{{ route('bearing.store') }}">
        @csrf
        <x-adminlte-input name="bearing" label="Bearing" placeholder="Enter Bearing" fgroup-class="mb-3" required/>
