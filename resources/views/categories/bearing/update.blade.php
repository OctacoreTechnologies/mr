...
@section('content')
    <div class="card">
        <div class="card-body">
            <form  action="{{ route('bearing.update', $bearing->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <x-adminlte-input type="text" name="bearing" value="{{$bearing->bearing}}" label="Bearing" fgroup-class="mb-3" required />
                    </div>
