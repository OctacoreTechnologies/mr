@php
    $heads = [
        ['label' => 'SR NO', 'width' => '7%'],
        'Machine','Model','Rotary Air Lock Valve',
...
@section('title', 'Rotary Air Lock Valve')

@section('content_header')
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        <h1 class="mb-2 mb-md-0">Rotary Air Lock Valve</h1>
        <div class="d-flex flex-wrap gap-2">
            {{-- Primary Add Button --}}
            <x-adminlte-button label="Add Rotary Air Lock Valve" theme="success" icon="fas fa-plus" data-toggle="modal" data-target="#modalMin"/>
...
<div class="card shadow-sm">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title">Rotary Air Lock Valve </h3>
    ...
    <div class="card-body">
        <x-adminlte-datatable id="table1" :heads="$heads" striped hoverable bordered compressed>
            @foreach ($rotaryAirLockValves as $rotaryAirLockValve)
                <tr>
                    <td>{{ $n++ }}</td>
                    <td>{{ $rotaryAirLockValve->model->machine->name ?? '' }}</td>
                    <td>{{ $rotaryAirLockValve->model->name ?? '' }}</td>
                    <td>{{ $rotaryAirLockValve->rotary_air_lock_valve ?? '' }}</td>
                    <td>
                        <nobr>
                            <a href="{{ route('rotary-air-lock-valves.edit', $rotaryAirLockValve->id) }}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit">
                                <i class="fa fa-lg fa-fw fa-pen"></i>
                            </a>
                            <form action="{{ route('rotary-air-lock-valves.destroy', $rotaryAirLockValve->id) }}" method="POST" class="d-inline-block">
                                @csrf
                                @method('DELETE')
...
<x-adminlte-modal id="modalMin" title="Add Material To Processs" theme="teal" icon="fas fa-plus">
    <form method="POST" action="{{ route('rotary-air-lock-valves.store') }}">
        @csrf
         <div class="col-12">
        ...
        <x-adminlte-input name="rotary_air_lock_valve" label="Rotary Air Lock Valve" placeholder="Enter Rotary Air Lock Valve" fgroup-class="mb-3" required/>
