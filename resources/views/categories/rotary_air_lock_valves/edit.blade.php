...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('rotary-air-lock-valves.update', $rotaryAirLockValve->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <x-adminlte-select name="machine_id" label="Select Machine" id="machine_id">
                            ...
                            @foreach ($machines as $machine)
                                <option value="{{ $machine->id }}"
                                    {{ $machine->id === ($rotaryAirLockValve->model->machine->id ?? null) ? 'selected' : '' }}>
                                    {{ $machine->name }}</option>
                            @endforeach
                        </x-adminlte-select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select id="model_id" name="model_id" class="form-control select2 form-control-lg"
                            ...
                            <option  selected value={{$rotaryAirLockValve->model_id}}>{{ $rotaryAirLockValve->model->name ?? '' }}</option>

                        </select>
                    </div>
                    <div class="col-md-6">
                        <x-adminlte-input type="text" name="rotary_air_lock_valve"
                            value="{{ $rotaryAirLockValve->rotary_air_lock_valve}}" label="Rotary Air Lock Valve(Hp)"
