...
@section('title', 'Blower')

@section('content_header')
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        <h1 class="mb-2 mb-md-0">Blower</h1>
        <div class="d-flex flex-wrap gap-2">
            {{-- Primary Add Button --}}
            <x-adminlte-button label="Add Blower" theme="success" icon="fas fa-plus" data-toggle="modal" data-target="#modalMin"/>
    ...
    <div class="card-body">
        <x-adminlte-datatable id="table1" :heads="$heads" striped hoverable bordered compressed>
            @foreach ($blowers as $blower)
                <tr>
                    <td>{{ $n++ }}</td>
                    <td>{{ $blower->model->machine->name ?? '' }}</td>
                    <td>{{ $blower->model->name ?? '' }}</td>
                    <td>{{ $blower->blower ?? '' }}</td>
                    <td>
                        <nobr>
                            <a href="{{ route('blowers.edit', $blower->id) }}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit">
                                <i class="fa fa-lg fa-fw fa-pen"></i>
                            </a>
                            <form action="{{ route('blowers.destroy', $blower->id) }}" method="POST" class="d-inline-block">
                                @csrf
                                @method('DELETE')
...
<x-adminlte-modal id="modalMin" title="Add Material To Processs" theme="teal" icon="fas fa-plus">
    <form method="POST" action="{{ route('blowers.store') }}">
        @csrf
         <div class="col-12">
        ...
        <x-adminlte-input name="blower" label="Blower(HP)" placeholder="Enter Blower" fgroup-class="mb-3" required/>
