@php
    $heads = [
        ['label' => 'SR NO', 'width' => '7%'],
        'Machine','Model','Feeding Hooper Capacity',
...
@section('title', 'feedingHooperCapacity')

@section('content_header')
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
        <h1 class="mb-2 mb-md-0">Feeding Hooper Capacity</h1>
        <div class="d-flex flex-wrap gap-2">
            {{-- Primary Add Button --}}
            <x-adminlte-button label="Add Feeding Hooper Capacity" theme="success" icon="fas fa-plus" data-toggle="modal" data-target="#modalMin"/>
...
<div class="card shadow-sm">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title">Feeding Hooper Capacity</h3>
    ...
    <div class="card-body">
        <x-adminlte-datatable id="table1" :heads="$heads" striped hoverable bordered compressed>
            @foreach ($feedingHooperCapacities as $feedingHooperCapacity)
                <tr>
                    <td>{{ $n++ }}</td>
                    <td>{{ $feedingHooperCapacity->model->machine->name ?? '' }}</td>
                    <td>{{ $feedingHooperCapacity->model->name ?? '' }}</td>
                    <td>{{ $feedingHooperCapacity->feeding_hooper_capacity ?? '' }}</td>
                    <td>
                        <nobr>
                            <a href="{{ route('feeding_hooper_capacities.edit', $feedingHooperCapacity->id) }}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit">
                                <i class="fa fa-lg fa-fw fa-pen"></i>
                            </a>
                            <form action="{{ route('feeding_hooper_capacities.destroy', $feedingHooperCapacity->id) }}" method="POST" class="d-inline-block">
                                @csrf
                                @method('DELETE')
...
<x-adminlte-modal id="modalMin" title="Add Material To Processs" theme="teal" icon="fas fa-plus">
    <form method="POST" action="{{ route('feeding_hooper_capacities.store') }}">
        @csrf
         <div class="col-12">
        ...
        <x-adminlte-input name="feeding_hooper_capacity" label="Feeding Hooper Capacity(Ltr)" placeholder="Enter feedingHooperCapacity" fgroup-class="mb-3" required/>
