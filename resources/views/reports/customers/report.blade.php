        ...
        <form id="filterForm" method="GET" action="{{ route('customer.report') }}" class="mb-4">
            <div class="row g-4">
                ...
                <div class="col-md-3">
                    ...
                    <select name="location_type[]" id="customer_source" class="form-control select2 rounded-pill"
                        multiple="multiple">
                ...
                <div class="col-md-3">
                    ...
                    <select name="region[]" id="region" class="form-control select2 rounded-pill" multiple="multiple">
                        @foreach ($regions as $region)
                            <option value="{{ $region}}" {{ in_array($region, request('region', [])) ? 'selected' : '' }}>
                                {{ $region}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="city[]" id="city" class="form-control select2 rounded-pill" multiple="multiple">
                        @foreach ($cities as $city)
                            <option value="{{ $city}}" {{ in_array($city, request('city', [])) ? 'selected' : '' }}>
                                {{ $city}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    ...
                    <select name="area[]" id="area" class="form-control select2 rounded-pill" multiple="multiple">
                        @foreach ($areas as $area)
                            <option value="{{ $area}}" {{ in_array($area, request('area', [])) ? 'selected' : '' }}>
                                {{ $area}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-3 mb-3 custom-date-range"
                    style="display: {{ request('due_date') == 'custom' ? 'block' : 'none' }};">
                    <label for="from_date" class="font-weight-semibold">From</label>
                    <input type="date" name="from_date" id="from_date" class="form-control rounded-pill"
                        value="{{ request('from_date') }}">

                    <label for="to_date" class="font-weight-semibold">To</label>
                    <input type="date" name="to_date" id="to_date" class="form-control rounded-pill"
                        value="{{ request('to_date') }}">
                </div>

                <div class="col-md-3">
                    <label for="followed_by" class="font-weight-bold text-muted">Followed By</label>
                    <select name="followed_by[]" id="followed_by" class="form-control select2 rounded-pill"
                        multiple="multiple">
                ...
                <div class="col-md-3">
                    <button type="submit" class="btn btn-primary btn-md mt-4 w-100 rounded-pill"
                        style="height: 40px;">Apply Filters</button>
                </div>
            </div>
        </form>
                    ...
                    <td>
                        <nobr>
                            <a href="{{route('customer.edit', $customer->id)}}"
                                class="btn btn-xs btn-outline-primary mx-1 shadow" title="Edit">
                                <i class="fa fa-pen"></i>
                            </a>
                            ...
                            <a href="{{route('customer.show', $customer->id)}}"
                                class="btn btn-xs btn-outline-success mx-1 shadow" title="Details">
                                <i class="fa fa-eye"></i>
                            </a>
                        </nobr>
                    </td>
