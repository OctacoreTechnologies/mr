...
<tr class="content bordered">
    <!-- LEFT COLUMN with INNER TABLE -->
<!-- LEFT COLUMN -->
<td class="left-col" style="vertical-align: top;">
    <table style="width: 100%; border:none; border-collapse: collapse; font-size: 12px;">
        <tr>
            <td colspan="2" style="padding: 5px; font-style: italic; font-weight: bold;">
                BILL TO :
            </td>
        </tr>
        <tr>
            <td style="width: 100%; padding: 5px; padding-bottom: 0;">
               M/s. {{ $saleOrder->quotation->customer->contact_person_1_name ?? '' }}
                <p>{{ $saleOrder->quotation->customer->address_line_1??'' }}</p>
            </td>
        </tr>
        <tr>
            <td style="padding: 5px; font-style: italic; font-weight: bold;">
                GST NO.:
            </td>
        ...
        <tr>
            <td style="width: 100%; padding: 5px; padding-bottom: 0; ">
                M/s.{{ $saleOrder->quotation->customer->contact_person_2_name ?? $saleOrder->quotation->customer->contact_person_1_name}}
                <p style="padding: 0;">{{ $saleOrder->quotation->customer->address_line_2?? $saleOrder->quotation->customer->address_line_1 }}</p>
            </td>
        </tr>
      {{--<tr>
        ...
        <tr>
            <td style="width:30%; padding: 5px; font-style: italic; font-weight: bold; ">
                Kind Attn.: Mr.
            </td>
        ...
        <tr>
            <td style=" padding: 5px; font-weight: bold;">Contact Detail:</td>
            <td style=" padding: 5px; ">
...
<!-- RIGHT COLUMN (Top block + divider + bottom aligned block) -->
<td class="right-col" style="vertical-align: top; padding:0;">
    <table style="width:100%; font-size:12px; border:1px solid #000; border-collapse:collapse; border: none;">
        <tr>
            <td style="padding:0; vertical-align:top;">
                <!-- TOP BLOCK -->
                <table style="width:100%; border-collapse:collapse; font-size:12px; border: none;">
                    <tr style="border: none;">
                        <td style="padding:8px 10px; font-weight:bold; font-style:italic; border: none;">
                            LAP NO.: 
                        </td>
                    ...
                    <tr style="border: none;">
                        <td style="padding:8px 10px; font-weight:bold; font-style:italic;">Quotation No.:</td>
                        <td style="padding:8px 10px;">
                            {{ $saleOrder->quotation->reference_no ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:8px 10px; font-weight:bold; font-style:italic;">Date:</td>
                        <td style="padding:8px 10px;">
        ...
        <!-- Middle Divider -->
        <tr >
            <td style="padding:0; border-top:1px solid #000; padding:0; height:10px; vertical-align:bottom;">
                <!-- BOTTOM BLOCK -->
                <table style="width:80%; border-collapse:collapse; font-size:12px; margin-top:auto; border: none;">
                    <tr style="border: none;">
                        <td style="padding:8px 10px; font-weight:bold; font-style:italic; border: none;">Customer P. O. No. :</td>
                        <td style="padding:8px 10px; border: none;">
                            {{ $saleOrder->quotation->customer->po_no ?? '' }}
                        </td>
                    </tr>
                    <tr style="border: none;">
                        <td style="padding:8px 10px; font-weight:bold; font-style:italic;border: none;">Date:</td>
                        <td style="padding:8px 10px; border: none;">
...
<table style="width: 100%; border-collapse: collapse; font-family: 'Poppins', sans-serif; font-size: 14px;border-top:hidden; ">
    <thead>
    ...
    <tbody>
        ...
        <tr>
            <td style="border: 1px solid black; padding: 6px;">
                <b>{{ strtoupper($saleOrder->quotation->machine->name) }} Model {{ $saleOrder->quotation->modele->name }} </b><br>
                <!-- ALONG WITH AC FREQUENCY DRIVE<br> -->
                <!-- ELECTRICAL PANEL<br><br> -->
                <br>
            </td>
            <td style="border: 1px solid black;">{{ $saleOrder->quoation->machine->hsn??'' }}</td>
            <td style="border: 1px solid black; text-align: center;">{{ $saleOrder->quotation->quantity??'' }}</td>
            ...
            @endphp
            {{--<td style="border: 1px solid black; text-align: right;">{{ format_indian_number($saleOrder->quotation->total_price) }}</td>--}}
            {{--<td style="border: 1px solid black; text-align: right;">{{ format_indian_number($saleOrder->discount) }}</td>--}}
            <td style="border: 1px solid black; text-align: right;">{{ format_indian_number($unitPrice/$saleOrder->quotation->quantity) }}</td>
...
<table style="width: 100%; border-collapse: collapse;   font-family: 'Poppins', sans-serif; font-size: 14px; border: 1px solid black;padding-bottom:0;">
    ...
    <tr>
        ...
        {{-- <td style="width: 15%; border: 1px solid black; text-align: right;">{{ format_indian_number($saleOrder->total_amount ??'0') }}</td> --}}
        <td style="width: 15%; border: 1px solid black; text-align: right;">{{ format_indian_number($unitPrice ??'0') }}</td>
    </tr>
    <tr>
      ...
      @if ($saleOrder->transporation_payment)
       ...
       <td style="border: 1px solid black; padding: 3px; text-align: right;">{{ format_indian_number($saleOrder->transporation_charge) }}</td>
       @else
       <td></td>
       <td></td>
      @endif
    ...
    <tr>
        ...
        <td colspan="2" style="border: 1px solid black; text-align: right;">{{ format_indian_number($saleOrder->insurance) }}</td>
    </tr>
    <tr>
        <td style="border: 1px solid black; padding: 5px;"><b><i>Packing: </i></b>Extra</td>
        <td colspan="2" style="border: 1px solid black; text-align: right;">{{ format_indian_number($saleOrder->packging) }}</td>
    </tr>
    <tr>
    ...
    <tr style="border-bottom: hidden;">
        <td style="border: 1px solid black; padding: 5px; border-bottom:none;"><b><i>Amount in Words:</i></b>{{strtoupper(convertToIndianWords($totalTax + $saleOrder->total_amount) ) }} RUPEES</td>
        <td style="border: 1px solid black; border-bottom: 2px; text-align: center; border-bottom:none;"><b>TOTAL<br>VALUE</b></td>
        {{--<td style="border: 1px solid black; text-align: right; border-bottom:none;">{{ format_indian_number($totalTax + $saleOrder->total_amount+$saleOrder->transporation_charge + $saleOrder->insurance + $saleOrder->packging ) }}</td>--}}
        <td style="border: 1px solid black; text-align: right; border-bottom:none;">{{ format_indian_number($totalTax + $unitPrice+$saleOrder->transporation_charge + $saleOrder->insurance + $saleOrder->packging ) }}</td>
    </tr>
  ...
  <tr style="height: 100px; line-height: 1.5; ">
    ...
    <td style="border-top: 1px solid black; padding: 10px;width: 30%; font-size: 14px; vertical-align: center; border-left: 1px solid black;">
